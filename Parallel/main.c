...
/* main function to run the full program. */
{
  ...
  // Input processed:
  parseArguments(argc, argv, &filename, &parameters);
  readFile(filename, &fullDataset, nprocs, myid);
  //preprocess(&fullDataset);

  //cleanData(&fullDataset);
  ...
  // We loop until no negative entries in beta:
	...
	run_serial_problem(&fullDataset, &alphOptProblem, &projectedSubProblem);
	...
	tradeInfo(&rd, &fullDataset, &nds, &alphOptProblem, &nfp, nprocs, myid, MPI_COMM_WORLD);

	if(myid == 0){
	  allocProjectedProblem(&nsp, nfp.p);
	}
	...
	int sending = 5;
	runParallelAlgorithm( &rd, &fullDataset, &alphOptProblem, &nds, &nfp, &nsp, dataWin, yWin, alphaWin, ytrWin, gradWin, myid, nprocs, MPI_COMM_WORLD, sending );
	if(myid == 0){
		printf("\nFinished Training!\n");
		freeYdata(&nds);
	}
